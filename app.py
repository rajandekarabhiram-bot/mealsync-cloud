...
# Default fallback meal schedule
fallback_data = {
...
# ============================================================================
# 3. DASHBOARD IMAGE RENDERER WITH DYNAMIC BATTERY ICON
# ============================================================================
...
def render_dashboard():
    ...
    # 800x600 Canvas for supersampling
    ...
    # ----------------------------------------------------
    # DYNAMIC BATTERY ICON & REMAINING TIME BADGE
    # ----------------------------------------------------
    batt_str = request.args.get('batt', '200d')  # e.g., '180d', '12h', 'CHG'
    batt_pct = int(request.args.get('pct', 95))   # e.g., 0 to 100

    # Draw remaining text (e.g., "180d")
    draw.text((640, 24), batt_str, font=font_badge, fill=255)

    # Battery Outline
    batX, batY = 724, 24
    draw.rectangle([batX, batY, batX + 44, batY + 24], outline=255, width=3)
    draw.rectangle([batX + 44, batY + 6, batX + 49, batY + 18], fill=255)  # Battery positive nipple

    if batt_str == "CHG":
        # Draw charging lightning bolt symbol inside battery
        draw.polygon([(batX + 22, batY + 3), (batX + 13, batY + 13), (batX + 21, batY + 13), (batX + 18, batY + 21), (batX + 31, batY + 10), (batX + 23, batY + 10)], fill=255)
    else:
        # Fill proportion according to percentage
