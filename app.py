...
# ============================================================================
def clamp_text(text, max_chars=MAX_PREP_CHARS):
    ...
    with get_db() as conn:
        ...
        if row:
            data = {
                ...
                "task1": clamp_text((row["task1"] or "—").replace("+", ",")),
                "task2": clamp_text((row["task2"] or "—").replace("+", ","))
            }
        else:
...
def api_menu_handler():
    ...
    with get_db() as conn:
        ...
        """, (day, str(req.get("breakfast", "")).replace("+", ","),
              ...
              clamp_text(str(req.get("task1", "")).replace("+", ",")),
              clamp_text(str(req.get("task2", "")).replace("+", ","))))
        conn.commit()
...
def home():
    ...
    <html>
    <head>
        <title>MealSync Production Display</title>
